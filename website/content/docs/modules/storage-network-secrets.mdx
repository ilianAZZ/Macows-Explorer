...
`network` permission. Outbound HTTP that runs **in Rust, not the WebView**, which
means it **bypasses CORS** — essential for talking to arbitrary servers (WebDAV,
S3, an API) that a browser fetch would block.
...
multi-account remote storage: `network` to reach the server, `secrets` for each
